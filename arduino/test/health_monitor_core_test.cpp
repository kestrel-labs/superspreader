...
TEST(TreatmentUpdateTests, SuperHealthyAbsorbing) {
  const HealthState init_health{.health=to_health(StateBounds::SUPER_HEALTHY), .cat_resistance=false};
  const auto next_health = treament_update(init_health);
  EXPECT_EQ(next_health.health, init_health.health);
}

TEST(TreatmentUpdateTests, HealthyToSuperHealthy) {
  const HealthState init_health{.health=to_health(StateBounds::HEALTHY), .cat_resistance=false};
  const auto next_health = treament_update(init_health);
...
TEST(TreatmentUpdateTests, InfectedAsymToHealthy) {
  const HealthState init_health{.health=to_health(StateBounds::INFECTED_ASYM), .cat_resistance=false};
  const auto next_health = treament_update(init_health);
  ASSERT_LT(next_health.health, init_health.health);
...
TEST(TreatmentUpdateTests, InfectedSymToHealthy) {
  const HealthState init_health{.health=to_health(StateBounds::INFECTED_SYM), .cat_resistance=false};
  const auto next_health = treament_update(init_health);
  ASSERT_LT(next_health.health, init_health.health);
...
TEST(TreatmentUpdateTests, InfectedSymLateToImmune) {
  const HealthState init_health{.health=to_health(StateBounds::INFECTED_SYM_LATE), .cat_resistance=false};
  const auto next_health = treament_update(init_health);
  ASSERT_LT(next_health.health, init_health.health);
...
TEST(TreatmentUpdateTests, ZombieStaysZombie) {
  const HealthState init_health{.health=to_health(StateBounds::ZOMBIE), .cat_resistance=false};
  const auto next_health = treament_update(init_health);
...
TEST(ExposureUpdateTests, NoExposureCausesNoChange) {
  const HealthState init_health{.health=to_health(StateBounds::HEALTHY), .cat_resistance=false};
  const auto next_health = exposure_update(init_health, ExposureEvent{
    .human = 0,
    .cat = 0,
  });
  ASSERT_EQ(next_health.health, init_health.health);
...
TEST(ExposureUpdateTests, ConsistentExposureCausesInfection) {
  ...
  {
    const auto next_health = exposure_update(curr_health, ExposureEvent{
      .human = 1,
      .cat = 0,
    });
...
TEST(ExposureUpdateTests, ConsistentExposureAfterInfectionCausesZombie) {
  HealthState curr_health{.health=to_health(StateBounds::INFECTED_ASYM), .cat_resistance=false};
...
TEST(ExposureUpdateTests, CatExposureCausesCatResistance) {
  ...
  for (int i = 0; i < kMaxIterToCatResist; ++i)
  {
    ...
    ASSERT_GT(next_health.health, curr_health.health);
    const bool cat_resistance_changed = next_health.cat_resistance != curr_health.cat_resistance;
    curr_health = next_health;
